...
def subscribe(client: mqtt_client):
    ...
    global velocity 
    global tProcess 
    def on_message(client, userdata, msg):  # xử lý sự kiện khi nhận được message từ MQTT broker.
        message = msg.payload.decode()      # trích xuất nội dung tin nhắn từ payload và chuyển đổi từ dạng byte sang chuỗi
        message = message[:-1]
        if 'STOP' in message: 
            tProcess = STOP 
            ...
            data = json.loads(message)      #chuyển đổi message thành đối tượng python
...
def doOpenCV(q, q2):
    ...
    lk_params = dict(winSize=(20, 20),      #phương pháp Optical Flow (Lucas-Kanade)
                    maxLevel=4,
    ...
    while True:
        if q2.qsize() == 2:
            ...
            if 7 in list_process: 
                ...
                list_process_backward.reverse()     # đảo ngược thứ tự các phần tử
            ...
            list_process_iter   = iter(list_process)    # iter cho phép duyệt qua từng phần tử
        ...
        if list_process: 
            ...
            for barcode in barcodes: 
                barcodeData = barcode.data.decode("utf-8")  #convert nhị phân -> chuỗi
                try: 
                ...
                (rv, points, straight_qrcode) = cv.QRCodeDetector().detectAndDecode(frame)  # rv: chứa dữ liệu giải mã
                if rv:
                    points = points[0]
                    ...
                    c = int(pt2[0])
                    d = int(pt1[0])
                    angle = atan2(b - a, c - d)     # góc beta trong CALIB3
                    angle = (angle * 180 / pi) 
                    ...
                    if count_send == 5:     # chưa hiểu lắm
                        ...
                        if tProcess == pNONE:
                            ...
                            angle_point = round(utils.ComputeAngle(p2_x, p2_y, p4_x, p4_y, T_x, T_y))   # góc alpha
                        ...
                        #                    
                            ...
                            if q.qsize() == 2 and q3.qsize() == 1:      # đoạn này là TH angle_point < 20
                                count_calib3 = 0 
                                print(q3.get())
                        ...
                        # Giữa hai hành động trong list_process là quá trình calib
                            ...
                            if count == 0: 
                                ...
                                if tProcess == 7 or tProcess == 8: 
                                    calib_mode = 1 
                                if tProcess == 12: 
                                    count = 1           # lý do vì sao count == 0 nằm ở dưới 
                                    calib_mode = 0 
                                    pre_message = None 
